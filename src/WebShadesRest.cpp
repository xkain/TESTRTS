...
namespace WebShadesRest {
  ...
  static void handleGetNextRoom(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
  ...
  static void handleGetNextShade(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
  ...
  static void handleGetNextGroup(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
  ...
  static void handleGetNextSchedule(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    JsonResponse resp;
  ...
  static void handleAddRoom(WebServer &server) {
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleAddShade(WebServer &server) {
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleAddGroup(WebServer &server) {
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleAddSchedule(WebServer &server) {
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleGroupOptions(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleSaveRoom(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleSaveShade(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleSaveGroup(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleSaveSchedule(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleLinkToGroup(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleUnlinkFromGroup(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleDeleteRoom(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleDeleteShade(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleDeleteGroup(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleDeleteSchedule(WebServer &server) {
    webServer.sendCORSHeaders(server);
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    HTTPMethod method = server.method();
  ...
  static void handleRoomSortOrder(WebServer &server) {
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    DynamicJsonDocument doc(512);
  ...
  static void handleShadeSortOrder(WebServer &server) {
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    DynamicJsonDocument doc(512);
    if(settings.enableDebugLogs) {
  ...
  static void handleGroupSortOrder(WebServer &server) {
    if(server.method() == HTTP_OPTIONS) { server.send(200, "OK"); return; }
    if(!webServer.isAuthenticated(server, true)) return;
    DynamicJsonDocument doc(512);
    if(settings.enableDebugLogs) {
  ...
  void registerRoutes(WebServer &server) {
    server.on("/rooms", [&server]() { handleGetRooms(server); });
    server.on("/shades", [&server]() { handleGetShades(server); });
    server.on("/groups", [&server]() { handleGetGroups(server); });
    server.on("/schedules", [&server]() { handleGetSchedules(server); });
    server.on("/room", [&server]() { handleRoom(server); });
    server.on("/shade", [&server]() { handleShade(server); });
    server.on("/group", [&server]() { handleGroup(server); });
    server.on("/schedule", [&server]() { handleSchedule(server); });
    server.on("/getNextRoom", [&server]() { handleGetNextRoom(server); });
    server.on("/getNextShade", [&server]() { handleGetNextShade(server); });
    server.on("/getNextGroup", [&server]() { handleGetNextGroup(server); });
    server.on("/getNextSchedule", [&server]() { handleGetNextSchedule(server); });
    server.on("/addRoom", [&server]() { handleAddRoom(server); });
    server.on("/addShade", [&server]() { handleAddShade(server); });
    server.on("/addGroup", [&server]() { handleAddGroup(server); });
    server.on("/addSchedule", [&server]() { handleAddSchedule(server); });
    server.on("/groupOptions", [&server]() { handleGroupOptions(server); });
    server.on("/saveRoom", [&server]() { handleSaveRoom(server); });
    server.on("/saveShade", [&server]() { handleSaveShade(server); });
    server.on("/saveGroup", [&server]() { handleSaveGroup(server); });
    server.on("/saveSchedule", [&server]() { handleSaveSchedule(server); });
    server.on("/linkToGroup", [&server]() { handleLinkToGroup(server); });
    server.on("/unlinkFromGroup", [&server]() { handleUnlinkFromGroup(server); });
    server.on("/deleteRoom", [&server]() { handleDeleteRoom(server); });
    server.on("/deleteShade", [&server]() { handleDeleteShade(server); });
    server.on("/deleteGroup", [&server]() { handleDeleteGroup(server); });
    server.on("/deleteSchedule", [&server]() { handleDeleteSchedule(server); });
    server.on("/roomSortOrder", [&server]() { handleRoomSortOrder(server); });
