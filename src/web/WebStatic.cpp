...
namespace WebStatic {
  ...
  // fichiers du pipeline de build (minify_data.py::minify_all(), qui gzippe systématiquement
  ...
  // minify_data.py::_embed_manifest -- lu directement par le backend C++, qui n'a pas de capacité
  ...
  // index.html, cf. minify_data.py::resolve_build_version) et qui peuvent donc être mis en cache
  ...
  void registerRoutes(AsyncWebServer &server) {
    ...
    // n'a d'effet réel qu'en build release (cf. minify_data.py::_set_build_cache_flag et le
    ...
    // base.css/main.css/overlays.css restent 3 fichiers éditables sous data-dev/, mais minify_data.py
